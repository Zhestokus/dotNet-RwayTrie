...
{
    public class RWayTrieDt<TVal> : IEnumerable<KeyValuePair<byte[], TVal>>
    {
        private RWayNodeDt<TVal> _root;
        ...
        public RWayTrieDt(Encoding encoding)
        {
            _encoding = encoding;
        }
        ...
        private RWayNodeDt<TVal> FindNode(byte[] key)
        {
            var node = _root;
        ...
        private RWayNodeDt<TVal> FindNode(String key)
        {
            var bytes = _encoding.GetBytes(key);
        ...
        {
            if (_root == null)
                _root = new RWayNodeDt<TVal> { Children = new Dictionary<byte, RWayNodeDt<TVal>>() };
            ...
            foreach (var b in key)
            {
                if (node.Children == null)
                    node.Children = new Dictionary<byte, RWayNodeDt<TVal>>();

                if (!node.Children.TryGetValue(b, out var rwNode))
                    rwNode = node.Children[b] = new RWayNodeDt<TVal>();
        ...
        private IEnumerable<(byte[], RWayNodeDt<TVal>)> PreOrderTraversal(byte key, RWayNodeDt<TVal> parent, List<byte> keyBuffer)
        {
            /*
        ...
        private IEnumerable<(byte[], RWayNodeDt<TVal>)> InOrderTraversal(byte key, RWayNodeDt<TVal> parent, List<byte> keyBuffer)
        {
            /*
        ...
        private IEnumerable<(byte[], RWayNodeDt<TVal>)> PostOrderTraversal(byte key, RWayNodeDt<TVal> parent, List<byte> keyBuffer)
        {
            /*
